...
본인이 리포스트한 게시물을 한 번에 Obsidian으로 동기화합니다.

1. 옵션 페이지 → **리포스트 일괄 가져오기** 섹션
2. **사용자명**(@ 제외) 입력 — `/@{username}/reposts` 페이지를 수집 대상으로 사용
3. **가져올 개수** 선택:
   - **전체** — 리포스트 끝까지 스크롤하며 전부 수집
   - **최근 10/20/30/60개** — 피드 위에서부터 N개만 수집
4. **일괄 가져오기 실행** → 새 탭이 열려 자동 스크롤·수집 후 순차 저장
5. 진행 상황은 옵션 페이지 하단 로그에 표시. 완료 시 `신규 / 덮어쓰기 / 실패` 요약 알림

> **날짜가 아니라 개수로 제어하는 이유**: Threads 리포스트 페이지는 "언제 리포스트했는지"를 노출하지 않습니다(원본 게시일만 있음). 따라서 "최근 3개월" 같은 날짜 필터는 정확히 구현할 수 없어, 피드의 **리포스트 최신순**을 신뢰해 위에서부터 개수로 가져옵니다.

- 이미 저장된 노트는 **덮어쓰기**, 새 URL은 **신규**로 처리됩니다.
- 가상 스크롤로 인한 누락을 막기 위해 화면 단위로 점진 스크롤합니다. 글이 많으면 수집에 수 분 걸릴 수 있습니다.

> **⚠️ 수집 탭을 화면 앞에 두세요**: 실행하면 리포스트 페이지 탭이 열립니다. Chrome은 백그라운드(가려진) 탭의 렌더링을 멈추기 때문에, 그 탭을 다른 탭/창으로 가리면 Threads가 글을 더 불러오지 않아 **일부만 수집**됩니다(예: 전체 452개인데 15개만). 탭이 가려지면 화면에 "⏸ 수집 일시정지" 안내가 뜨고, 다시 앞으로 두면 자동 재개합니다.
...
└── 2026-05-04 [사업하는 스친이들 주목]/
    ...
    ├── img1.jpg
    ├── img2.jpg
    ├── vid1-poster.jpg   ← 파일로 못 받은 동영상의 썸네일
...
> [!info] 동영상 (0:42)
> ![[vid1-poster.jpg]]
> 스트리밍 전용(blob:/HLS)이라 동영상 파일을 저장하지 못했습니다 — [원본에서 보기](https://www.threads.com/@user/post/abc123)
```

### 동영상이 있는 게시물

Threads 동영상은 대부분 브라우저 안에서 조각(DASH)으로 재조립돼 재생되고, 이때 `<video>`의 주소는
`blob:…`이라 **파일로 다시 내려받을 수 없습니다**. 이 확장은 그 경우 썸네일과 재생 길이, 원본 게시물
링크를 노트에 남깁니다. 반대로 게시물이 `https://…mp4` 주소를 그대로 노출하면 `vidN.mp4`로 저장하고
`![[vidN.mp4]]`로 노트에서 바로 재생할 수 있습니다.

**동영상이 있었는데 노트에 아무 표시도 없는 경우는 없습니다.** 파일로 저장하지 못한 개수는 노트 끝의
`<!-- 동영상 N개 파일 저장 실패 — 링크만 기록 -->` 주석으로 확인할 수 있습니다.
...
- **동영상 원본 저장 제한**: 스트리밍(blob:/HLS/DASH) 재생분은 파일로 저장 불가 → 썸네일 + 원본 링크만 남음. 자막·오디오 트랙·화질 선택은 수집하지 않음
- **미디어 URL 만료**: 노트에 남는 원본 동영상 URL은 서명이 붙어 있어 시간이 지나면 열리지 않음 (원본 게시물 링크는 계속 유효)
- **HTTPS 자체서명 인증서**: Chrome service worker fetch에서 막히는 경우가 흔함 → HTTP 27123 사용 권장
- **리포스트 날짜 필터 불가**: 리포스트 시점이 페이지에 없어 날짜 기준 수집은 불가능 → 개수(전체/최근 N개) 기준으로만 제어
